...
impl Display for ReadClipboard<'_> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		let b64_mime = STANDARD.encode(self.mime);
		let mut metadata = String::new();
		if self.pw.len() > 0 {
			let b64_pw = STANDARD.encode(self.pw);
			let b64_name = STANDARD.encode(self.name);
...
impl Display for WriteClipboard<'_> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		for item in &self.data {
			let b64_mime = STANDARD.encode(item.mime);
			...
			if item.alias.len() > 0 {
				let b64_alias = STANDARD.encode(item.alias);
