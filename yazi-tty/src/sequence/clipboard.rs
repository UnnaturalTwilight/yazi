...
impl SetClipboard {
	pub fn new(content: impl AsRef<[u8]>) -> Self {
		Self { content: STANDARD.encode(content) }
	}
}
...
impl Display for ReadClipboard<'_> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		let b64_mime = STANDARD.encode(self.mime).into_bytes();
		let mime_str = unsafe { String::from_utf8_unchecked(b64_mime) };
		let mut metadata = String::new();
		if self.pw.len() > 0 {
			let b64_pw = STANDARD.encode(self.pw).into_bytes();
			let pw_str = unsafe { String::from_utf8_unchecked(b64_pw) };
			let b64_name = STANDARD.encode(self.name).into_bytes();
...
impl Display for ReadClipboardMimes {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		write!(f, "\x1b]5522;type=read;{}\x1b\\", STANDARD.encode(b"."))
	}
}
...
impl Display for WriteClipboard<'_> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		for item in &self.data {
			let b64_mime = STANDARD.encode(item.mime).into_bytes();
			let mime_str = unsafe { String::from_utf8_unchecked(b64_mime) };
			let data = item.payload;

			for (_, chunk) in data.chunks(4096).enumerate() {
				let b64_chunk = STANDARD.encode(chunk).into_bytes();
				let s = unsafe { String::from_utf8_unchecked(b64_chunk) };
				write!(f, "\x1b]5522;type=wdata:mime={};{s}\x1b\\", mime_str)?;
			}

			if item.alias.len() > 0 {
				let b64_alias = STANDARD.encode(item.alias).into_bytes();
